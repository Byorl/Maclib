...
function MacLib:Window(Settings)
	...
	local function applyState(button, enabled)
		...
		local transparency = enabled and windowControlSettings.transparencies.enabled
			or windowControlSettings.transparencies.disabled
	...
	local function ChangeState(State)
		Tween(divider, TweenInfo.new(0.2, TweenSettings.EasingStyle), {
			BackgroundTransparency = State == "Idle" and TweenSettings.DefaultTransparency
				or TweenSettings.HoverTransparency,
		}):Play()
	end
		...
		local tweens = {
			checkIn = Tween(checkmark, TweenInfo.new(tweensettings.duration, tweensettings.easingStyle), {
				...
				Size = UDim2.new(
					...
					checkmark.Size.Y.Scale,
					checkmark.Size.Y.Offset
				),
			}),
	...
	function WindowFunctions:TabGroup()
		...
		function SectionFunctions:Tab(Settings)
			...
			function TabFunctions:Section(Settings)
				...
				local sectionTargets = TabFunctions._SectionTargets
				local leftTarget = sectionTargets and sectionTargets.Left or left
				local rightTarget = sectionTargets and sectionTargets.Right or right
				section.Parent = Settings.Side == "Left" and leftTarget or rightTarget
				...
				function SectionFunctions:Slider(Settings, Flag)
					...
					local DisplayMethods = {
						...
						Degrees = function(sliderValue, precision)
							local formattedValue = precision and string.format("%." .. precision .. "f", sliderValue)
								or tostring(sliderValue)
						...
						Percent = function(sliderValue, precision)
							local percentage = (sliderValue - SliderFunctions.Settings.Minimum)
								...
								* 100
							return precision and string.format("%." .. precision .. "f", percentage) .. "%"
								or tostring(math.round(percentage)) .. "%"
						end,
						LiteralPercent = function(sliderValue, precision)
							local formattedValue = precision and string.format("%." .. precision .. "f", sliderValue)
								or tostring(math.round(sliderValue))
							return formattedValue .. "%"
						end,
						Value = function(sliderValue, precision)
							return precision and string.format("%." .. precision .. "f", sliderValue)
								or tostring(sliderValue)
						end,
					}

					local ValueDisplayMethod = DisplayMethods[SliderFunctions.Settings.DisplayMethod]
						or DisplayMethods.Value
					local finalValue
					...
					local function SetValue(val, ignorecallback)
						...
						if typeof(val) == "Instance" then
							local input = val
							local barPosition = dragBarPosition or sliderBar.AbsolutePosition.X
							local barWidth = dragBarWidth or sliderBar.AbsoluteSize.X
					...
					sliderHead.InputBegan:Connect(function(input)
						if
							...
							or input.UserInputType == Enum.UserInputType.Touch
						then
							dragBarPosition = sliderBar.AbsolutePosition.X
							dragBarWidth = sliderBar.AbsoluteSize.X
							dragging = true
							if SliderFunctions.Settings.onInputBegan then
								SliderFunctions.Settings.onInputBegan(finalValue)
							end
							SetValue(input)
						end
					end)
					...
					sliderValue.FocusLost:Connect(function(enterPressed)
						...
						if value then
							...
							if isPercent and SliderFunctions.Settings.DisplayMethod ~= "LiteralPercent" then
				...
				function SectionFunctions:Input(Settings, Flag)
					...
					inputName.AnchorPoint = Vector2.zero
					...
					inputName.Position = UDim2.fromOffset(0, 5)
					...
					local CharacterSubs = {
						...
						Numeric = function(value)
							local result = value:match("^%-?%d*$") and value
								or value:gsub("[^%d-]", ""):gsub("(%-)", function(match, pos)
					...
					if type(InputFunctions.Settings.AcceptedCharacters) == "function" then
						...
						AcceptedCharacters = CharacterSubs[InputFunctions.Settings.AcceptedCharacters]
					...
					checkSize()
					Input:GetPropertyChangedSignal("AbsoluteSize"):Connect(checkSize)
					...
					UserInputService.InputBegan:Connect(function(inp)
						if focused and not isBinding then
							...
							Event = UserInputService.InputBegan:Connect(function(input)
								if
									KeybindFunctions.Settings.Blacklist
									and (
										table.find(KeybindFunctions.KeybindFunctions.Settings.Blacklist, input.KeyCode)
										or table.find(KeybindFunctions.Settings.Blacklist, input.UserInputType)
									)
								then
									binderBox:ReleaseFocus()
									resetFocusState()
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					dropdownName.Text = Settings.Default
							and (DropdownFunctions.Settings.Name .. " • " .. table.concat(Selected, ", "))
						or (DropdownFunctions.Settings.Name .. "...")
								...
								for name, opt in pairs(OptionObjs) do
									if name ~= optionName then
										Tween(
											...
											{
												Size = UDim2.new(
													opt.Checkmark.Size.X.Scale,
													tweensettings.checkSizeDecrease,
													opt.Checkmark.Size.Y.Scale,
													opt.Checkmark.Size.Y.Offset
												),
											}
										):Play()
										Tween(
					...
					local function ToggleDropdown()
						...
						local targetSize = isDropdownOpen and UDim2.new(1, 0, 0, CalculateDropdownSize())
							or UDim2.new(1, 0, 0, defaultDropdownSize)

						local dropTween = Tween(
							dropdown,
							TweenInfo.new(0.2, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out),
							{
								Size = targetSize,
						...
						local iconTween =
							Tween(dropdownImage, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				...
				function SectionFunctions:Colorpicker(Settings, Flag)
					...
					confirm.MouseButton1Click:Connect(function()
						...
						if ColorpickerFunctions.Settings.Callback then
							task.spawn(function()
								ColorpickerFunctions.Settings.Callback(
									ColorpickerFunctions.Color,
									isAlpha and ColorpickerFunctions.Alpha
								)
							end)
						end
					end)
			...
			local subTabGroupFunctions

			function TabFunctions:SubTabGroup()
				if subTabGroupFunctions then
					return subTabGroupFunctions
				end
				...
				local function preparePage(page, settings)
					...
					local pageRight = page:FindFirstChild("Right")
					assert(pageLeft and pageRight, "subtab page columns are unavailable")
					local columns = tonumber(settings.Columns) == 1 and 1 or 2
					if columns == 1 then
						pageLeft.Size = UDim2.new(1, -4, 0, 0)
						pageRight.Visible = false
					end
					return pageLeft, pageRight, columns
				end

				function SubTabGroupFunctions:SubTab(Settings)
					Settings = Settings or {}
					assert(type(Settings.Name) == "string" and Settings.Name ~= "", "subtab requires a name")
					subTabIndex += 1

					local page
					if firstPageAvailable then
						firstPageAvailable = false
						page = elementsScrolling
					else
						page = pageTemplate:Clone()
					end
					local pageLeft, pageRight, columns = preparePage(page, Settings)
					...
					function SubTabFunctions:Section(sectionSettings)
						sectionSettings = table.clone(sectionSettings or {})
						if columns == 1 then
							sectionSettings.Side = "Left"
						end
						local previousTargets = TabFunctions._SectionTargets
						TabFunctions._SectionTargets = { Left = pageLeft, Right = pageRight }
						local created = TabFunctions:Section(sectionSettings)
						TabFunctions._SectionTargets = previousTargets
						return created
					end
					...
					switcher.MouseButton1Click:Connect(function()
						SubTabFunctions:Select()
					end)
					if not selectedSubTab then
						SubTabFunctions:Select()
					end
					return SubTabFunctions
				end

				function SubTabGroupFunctions:GetSelected()
					return selectedSubTab
				end

				subTabGroupFunctions = SubTabGroupFunctions
				return SubTabGroupFunctions
			end

			function TabFunctions:SubTab(Settings)
				return self:SubTabGroup():SubTab(Settings)
			end
	...
	function WindowFunctions:Notify(Settings)
		...
		local style = styles[Settings.Style] or function()
			interactable:Destroy()
		end
		style()
	...
	function MacLib:RefreshConfigList()
		if isStudio or not (isfolder and listfiles) then
			return "Config system unavailable."
		end

		local list = (isfolder(MacLib.Folder) and isfolder(MacLib.Folder .. "/settings"))
				and listfiles(MacLib.Folder .. "/settings")
			or {}
